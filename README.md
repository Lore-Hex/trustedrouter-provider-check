...
| 4 | HTTP status before streaming, strict enclave-readable SSE framing, in-band errors, `[DONE]`, usage, first meaningful delta deadline, incremental delivery, and capability-gated signed receipt wire/signature/request/response/nonce/issuer bindings | Silent/truncated output, missing usage, missed output budget, errors embedded after HTTP 200, and invalid declared receipts are `fail`. Missing `[DONE]` and whole-body buffering are `warn` because the enclave tolerates them but they remain risky. Receipt checks skip without a Catalog v2 receipts declaration. |
